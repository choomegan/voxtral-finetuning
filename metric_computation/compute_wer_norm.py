...
def normalize_and_compute_wer(manifest_path: str, remove_unk: bool = True) -> None:
    ...
    print(
        "Percentage of samples with <UNK> in reference text: ",
        round((df["text"].apply(lambda x: "<UNK>" in x).sum() / len(df) * 100), 2),
        "%",
    )
    ...
    predictions = df["prediction"].apply(normalize_text).apply(_remove_unk).tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--remove_unk",
        action="store_true",
        help="Whether to remove samples with <UNK> from references before computing WER.",
    )
